...
/**
 * For a given image path, return the raw path (works for local uploads)
 * or the resolved backend URL (for absolute URLs already in the value).
 */
function displaySrc(path: string | null | undefined): string {
  if (!path) return "";
  if (path.startsWith("http://") || path.startsWith("https://")) return path;
  return path; // use raw path — Next.js serves from public/
}
